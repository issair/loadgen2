...
struct LoadableSampleSet {
  ...
  std::function<std::vector<QuerySampleIndex>(QuerySampleIndex)> get_dependency_chain;
};
...
struct ResponseDelegateDetailed : public ResponseDelegate {
  ...
  void SampleComplete(SampleMetadata* sample, QuerySampleResponse* response,
    ...
    Log([sample, complete_begin_time, sample_data_copy,
      ...
      auto sample_start = (sample->sample_start_time != PerfClock::time_point::min())
                              ? sample->sample_start_time
                              : query->issued_start_time;
      if (scenario == TestScenario::Server) {
        // Trace the server scenario as a stacked graph via counter events.
        DurationGeneratorNs issued{sample_start};
        log.TraceCounterEvent("Latency", query->scheduled_time, "issue_delay",
                              sched.delta(sample_start),
                              "issue_to_done",
      ...
      log.TraceSample("Sample", sample->sequence_id, query->scheduled_time,
                      ...
                      sched.delta(sample_start), "complete_ns",
      ...
      QuerySampleLatency latency = 
          (sample->sample_start_time != PerfClock::time_point::min())
              ? std::chrono::duration_cast<std::chrono::nanoseconds>(
                    complete_begin_time - sample->sample_start_time).count()
              : sched.delta(complete_begin_time);
    ...
    Log([sample, complete_begin_time, token_data_copy](AsyncLog& log) {
      ...
      if (scenario == TestScenario::Server) {
        ...
        log.TraceCounterEvent(
            ...
            sched.delta(sample_start), "issue_to_done",
            issued.delta(complete_begin_time));
      } else {
...
std::vector<QueryMetadata> GenerateQueries(
  ...
  while (prev_timestamp < gen_duration || queries.size() < min_queries) {
    ...
    // Expand dependency chain: replace the single sample with the full
    // dependency chain.  Each dependency becomes a separate sample in the
    // query and gets independent latency measurement by LoadGen.
    if (settings.enable_dependency_replay &&
        loaded_sample_set.get_dependency_chain) {
      // Only expand when there is exactly one sample (Server scenario)
      if (samples.size() >= 1) {
        auto chain = loaded_sample_set.get_dependency_chain(samples[0]);
        if (chain.size() > 1) {
          samples.assign(chain.begin(), chain.end());
        }
      }
    }
...
std::vector<LoadableSampleSet> GenerateLoadableSets(
  ...
  for (auto s : samples) {
    ...
    if (loadable_set.size() == set_size) {
      ...
      result.back().get_dependency_chain = [qsl](QuerySampleIndex idx) -> std::vector<QuerySampleIndex> {
        return qsl->GetDependencyChain(idx);
      };
      loadable_set.clear();
      loadable_set.reserve(set_size + set_padding);
    }
...
///
/// \brief Mark the start time for sample(s) identified by response->id.
void QuerySampleStart(QuerySampleResponse* responses, size_t response_count) {
  using namespace loadgen;
  for (size_t i = 0; i < response_count; i++) {
    SampleMetadata* sample =
        reinterpret_cast<SampleMetadata*>(responses[i].id);
    sample->sample_start_time = PerfClock::now();
  }
}
