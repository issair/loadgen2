...
struct TestSettings {
  ...
  /// \brief When true, LoadGen queries the QSL for dependency chains
  /// via QuerySampleLibrary::GetDependencyChain() and expands each
  /// sampled index into a multi-sample query. Each dependency sample
  /// gets independent latency measurement by LoadGen.
  bool enable_dependency_replay = false;
