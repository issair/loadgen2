...
BACKEND_REGISTRY = {
    ...
    "sglang": {
        ...
        "config": {
            ...
            "speculative_algorithm": "EAGLE",  # EAGLE style decoding with DS-R1 MTP Head
            "speculative_num_steps": 3,
            "speculative_topk": 1,  # Linear (no draft-trees)
            # External server mode: when set, skip launching local server
            # and connect to an already-running SGLang server.
            # Set via environment variable OPENAI_API_BASE
            # e.g. "http://10.0.0.1:30000"
            "external_server_url": os.environ.get("OPENAI_API_BASE", None),
            "external_server_api_key": os.environ.get("OPENAI_API_KEY", None),
        },
        "env_vars": {
            "CUDA_MODULE_LOADING": "LAZY",
            "NCCL_TREE_THRESHOLD": "0",
...
def _get_compatibility_error_message(
    backend: str, runner_type: str, compatible: List[str]
) -> str:
    """
...
def validate_runner_for_backend(runner_type: str) -> str:
    ...
    if not is_backend_compatible_with_runner(backend, runner_type):
        ...
        error_msg = _get_compatibility_error_message(backend, runner_type, compatible)
...
def is_backend_compatible_with_runner(
    backend_name: Optional[str] = None, runner_type: str = None
) -> bool:
...
def get_backend_env_vars(backend_name: Optional[str] = None) -> Dict[str, str]:
    ...
    if backend_name == "vllm":
        ...
        env_vars["OMP_NUM_THREADS"] = str(config.get("tensor_parallel_size", 8))
