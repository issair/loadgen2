...
class SGLangBackend(BaseBackend):
    ...
    Supports two modes:
    1. Local server mode (default): Launches and manages its own SGLang server.
    2. External server mode: Connects to an already-running SGLang server.
       Activate by setting SGLANG_EXTERNAL_SERVER_URL env var.
    """
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self._external_server_url = self.config.get("external_server_url", None)
        self._external_server_api_key = self.config.get("external_server_api_key", None)
        self._external_mode = self._external_server_url is not None

        # Dynamic port allocation to avoid conflicts (only for local mode)
        if not self._external_mode:
            self.port = find_free_port(30000)
            self.config["port"] = self.port
        else:
            # In external mode, extract port from URL if present
            self.port = None
            if self._external_server_url:
                from urllib.parse import urlparse

                parsed = urlparse(self._external_server_url)
                if parsed.port:
                    self.port = parsed.port

        # Server process management (only used in local mode)
        ...
        # Log monitoring (only used in local mode)
    ...
    def _check_server_alive(self) -> None:
        ...
        In external mode, this is a no-op since we don't manage the server.
        """
    ...
    def _build_server_command(self) -> List[str]:
        ...
        if self.config["enable_speculative_decode"]:
            cmd.extend(
                ["--speculative-algorithm", self.config["speculative_algorithm"]]
            )
            cmd.extend(
                ["--speculative-num-steps", str(self.config["speculative_num_steps"])]
            )
            cmd.extend(
                ["--speculative-eagle-topk", str(self.config["speculative_topk"])]
            )
        ...
        cmd.extend(["--max-running-requests", str(self.config["max_running_requests"])])
    ...
    def _start_server(self) -> None:
        ...
        self.server_log_file = (
            log_dir / f"sglang_server_{self.port}_{int(time.time())}.log"
        )
    ...
    def _verify_external_server(self) -> None:
        """Verify connectivity to the external SGLang server."""
        assert self._external_server_url is not None
        server_url: str = self._external_server_url.rstrip("/")

        print("\n[SGLANG] " + "=" * 60)
        print("[SGLANG] External server mode ENABLED")
        print(f"[SGLANG] Server URL: {self._external_server_url}")
        print("[SGLANG] " + "=" * 60)

        # Check health endpoint
        health_url = f"{server_url}/health"
        print(f"[SGLANG] Checking health at: {health_url}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    print(f"[SGLANG] ✓ External server health check passed!")
                    break
                else:
                    print(
                        f"[SGLANG] Health check returned status {response.status_code}, retrying..."
                    )
            except requests.exceptions.RequestException as e:
                print(
                    f"[SGLANG] Health check attempt {attempt + 1}/{max_retries} failed: {e}"
                )
                if attempt < max_retries - 1:
                    time.sleep(3)
                else:
                    raise RuntimeError(
                        f"Cannot reach external SGLang server at {self._external_server_url}. "
                        f"Make sure the server is running and accessible."
                    )

        # Run a warmup query
        print("[SGLANG] Running warmup query against external server...")
        try:
            base_url = f"{server_url}/v1"
            api_key = self._external_server_api_key or "dummy-key"
            ...
            if warmup_response.choices[0].message.content:
                print("[SGLANG] ✓ Warmup query to external server successful!")
                print(
            ...
            raise RuntimeError(
                f"Warmup query to external server failed: {e}\n"
                f"Make sure the server is fully ready."
            )

    def initialize(self) -> None:
        """Initialize the SGLang backend.

        In local mode: starts an SGLang server and connects to it.
        In external mode: connects to an already-running external server.
        """
        ...
        try:
            ...
            # Support TOKENIZER_PATH env var for local tokenizer directory
            model_path = os.environ.get("TOKENIZER_PATH", self.config["tokenizer"])
            local_only = (
                "TOKENIZER_PATH" in os.environ
                or os.environ.get("TRANSFORMERS_OFFLINE", "") == "1"
            )
            print(
                f"[SGLANG] Loading tokenizer: {model_path} (local_files_only={local_only})..."
            )
            ...
            if self._external_mode:
                # ---- External server mode: skip starting server ----
                print(
                    f"\n[SGLANG] External server mode: connecting to {self._external_server_url}"
                )

                # Verify external server is alive and ready
                self._verify_external_server()

                # Use the external server URL directly
                assert self._external_server_url is not None
                server_url: str = self._external_server_url.rstrip("/")
                base_url = f"{server_url}/v1"
                api_key = self._external_server_api_key or "dummy-key"
                ...
                print("[SGLANG] Created synchronous OpenAI client")
                ...
                print("[SGLANG] Created asynchronous OpenAI client")
                ...
                print(
                    "[SGLANG] Backend initialized successfully (external server mode)!"
                )
            else:
                # ---- Local server mode: start server ----
                self._start_server()
            ...
            # Stop server if it was started (local mode only)
            if not self._external_mode:
                self._stop_server()
    ...
    async def _async_generate_single(
        self, prompt: str, idx: int, semaphore: asyncio.Semaphore
    ...
    def shutdown(self) -> None:
        ...
        # Stop server (only in local mode; external server is managed externally)
        if not self._external_mode:
            self._stop_server()
        else:
            print(
                "[SGLANG] External server mode: server is managed externally, not stopping."
            )
