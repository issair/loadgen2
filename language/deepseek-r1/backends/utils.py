...
def get_cache_directory() -> Path:
    """
    Get the cache directory. Tries /raid/data/$USER/.cache first,
    falls back to ./.cache/huggingface in the current directory.
    ...
    # Try /raid/data/$USER/.cache first
    ...
    # Fall back to ./.cache/huggingface in current directory
    fallback = Path.cwd() / ".cache" / "huggingface"
...
def validate_prompts(
    tokenized_prompts: Optional[list] = None,
...
class LogMonitor:
    ...
    def __init__(
        ...
        self.header_text = (
            header_text or f"Server startup logs (last {max_lines} lines):"
        )
    ...
    def start(
        self, wait_for_file: bool = True, file_wait_timeout: float = 30.0
    ) -> bool:
        """
    ...
    def _setup_display_area(self) -> None:
        ...
        print(
            TerminalDisplay.MOVE_CURSOR_UP.format(self.total_lines), end="", flush=True
        )
    ...
    def _update_display(self, line_buffer: list) -> None:
        ...
        # our area)
        print(
            TerminalDisplay.MOVE_CURSOR_UP.format(self.total_lines + 1),
            end="",
            flush=True,
        )
