...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    # CLI overrides for user.conf (generate a temporary user.conf)
    parser.add_argument(
        "--target-qps",
        type=float,
        default=None,
        help="Override *.Offline.target_qps or *.Server.target_qps",
    )
    parser.add_argument(
        "--min-duration",
        type=int,
        default=None,
        help="Override *.{scenario}.min_duration (ms)",
    )
    parser.add_argument(
        "--min-query-count",
        type=int,
...
def run_loadgen_test(
    sut: Union[OfflineSUT, ServerSUT],
...
def main():
    ...
    try:
        ...
        if args.log_dir:
            ...
            log_dir = (
                output_dir / mode / ("accuracy" if args.accuracy else "performance")
            )
        ...
        if uses_text_prompts:
            ...
            dataset_for_sut = tokenized_prompts
            strings_for_sut = (
                processed_strings  # This is what gets used for generation now
            )
        ...
        with backend:
            ...
            # Determine scenario string for temp user.conf
            scenario = "Offline" if args.mode == "offline" else "Server"

            # If CLI overrides are provided, generate a temporary user.conf
            ...
            final_user_conf = temp_user_conf if temp_user_conf else args.user_conf
            ...
            try:
                ...
                processed_results = process_mlperf_results(sut_results, tokenizer)
