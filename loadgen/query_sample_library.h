...
class QuerySampleLibrary {
 public:
  ...
  /// \brief Returns the dependency chain for a given sample index.
  /// When enable_dependency_replay is true in TestSettings, this returns
  /// the ordered list of sample indices that must be processed sequentially
  /// as part of the same query.  Each index in the chain becomes a separate
  /// sample in the issued query and gets its own independent latency
  /// measurement by LoadGen.
  /// The default implementation returns just the sample itself (no deps).
  virtual std::vector<QuerySampleIndex> GetDependencyChain(
      QuerySampleIndex sample_index) const {
    return {sample_index};
  }
};
