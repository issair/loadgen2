...
struct SampleMetadata {
  ...
  /// Per-sample start time, set by SUT via QuerySampleStart.
  /// Default PerfClock::time_point::min() means "not set";
  /// when set, SampleComplete uses this instead of query start.
  PerfClock::time_point sample_start_time = PerfClock::time_point::min();
};
