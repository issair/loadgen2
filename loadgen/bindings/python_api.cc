...
using UnloadSamplesFromRamCallback =
    std::function<void(std::vector<QuerySampleIndex>)>;
using GetDependencyChainCallback =
    std::function<std::vector<QuerySampleIndex>(QuerySampleIndex)>;
...
class QuerySampleLibraryTrampoline : public QuerySampleLibrary {
 public:
  QuerySampleLibraryTrampoline(
      ...
      GetDependencyChainCallback get_dependency_chain_cb = nullptr)
      : name_(std::move(name)),
        ...
        get_dependency_chain_cb_(get_dependency_chain_cb) {}
  ...
  std::vector<QuerySampleIndex> GetDependencyChain(
      QuerySampleIndex sample_index) const override {
    if (get_dependency_chain_cb_) {
      pybind11::gil_scoped_acquire gil_acquirer;
      return get_dependency_chain_cb_(sample_index);
    }
    return {sample_index};
  }
 ...
 private:
  ...
  GetDependencyChainCallback get_dependency_chain_cb_;
};
...
void QuerySampleStart(std::vector<QuerySampleResponse> responses) {
  pybind11::gil_scoped_release gil_releaser;
  mlperf::QuerySampleStart(responses.data(), responses.size());
}
...
PYBIND11_MODULE(mlperf_loadgen, m) {
  ...
  pybind11::class_<TestSettings>(m, "TestSettings")
      ...
      .def_readwrite("enable_dependency_replay",
                     &TestSettings::enable_dependency_replay)
  ...
  m.def("FirstTokenComplete", &py::FirstTokenComplete,
        ...
        pybind11::arg("responses"),
        pybind11::arg("response_cb") = ResponseCallback{});
  m.def("QuerySampleStart", &py::QuerySampleStart,
        "Called by the SUT to mark the start of processing for individual"
        "samples, enabling independent latency measurement per sample.",
        pybind11::arg("responses"));
}
